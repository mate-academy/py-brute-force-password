...
def check_password(cpu_power_range) -> tuple:
    found_passwords = []
    counter = 0
    ...
    return found_passwords, counter
...
def brute_force_password() -> None:
    ...
    for future in futures:
        found, amount = future.result()
        amount_of_all += amount
        found_passwords.extend(found)
        if len(found_passwords) >= 10:
            break
    print(f"RESULT: {list(found_passwords)}\nCHECKED: {amount_of_all} PASSWORDS")
