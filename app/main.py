...
def brute_force(start: int, end: int) -> None:
    for password in range(start, end):
        formated_password = "{:08d}".format(password)
        if sha256_hash_str(formated_password) in PASSWORDS_TO_BRUTE_FORCE:
            print(f"{sha256_hash_str(formated_password)}: {formated_password}")
